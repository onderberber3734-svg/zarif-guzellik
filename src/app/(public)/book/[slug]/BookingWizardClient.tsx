...
import { getPublicServices, getPublicStaffForService, getPublicAvailability, createPublicBooking } from "@/app/actions/public-booking";
...
export default function BookingWizardClient({ business }: { business: any }) {
    ...
    const handleServiceSelect = async (serviceId: string) => {
        ...
        if (res.success && res.data && res.data.length > 0) {
            ...
            // Hiç personel yoksa direkt tarihe atla veya hata ver. Biz şimdilik hata verelim.
            alert("Bu hizmeti veren personel bulunmamaktadır.");
        }
        setIsLoading(false);
    };
    ...
    const handleCustomerSubmit = async (e: React.FormEvent) => {
        ...
        if (res.success) {
            ...
            setErrorMsg(res.error || "Randevu oluşturulamadı.");
        }
    };
        ...
        return (
            <div className="animate-in fade-in slide-in-from-right-8 duration-300">
                ...
                <form onSubmit={handleCustomerSubmit} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                    ...
                    {errorMsg && (
                        <div className="p-3 bg-rose-50 border border-rose-200 text-rose-600 text-xs font-bold rounded-xl mt-4 text-center">
                            {errorMsg}
                        </div>
                    )}
        ...
        return (
            <div className="animate-in fade-in zoom-in-95 duration-500 h-full flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm w-full max-w-sm mx-auto mb-8">
                    ...
                    <p className="text-sm text-slate-500 mt-1 flex items-center justify-center gap-1"><span className="material-symbols-outlined text-[16px]">location_on</span> Bizi tercih ettiğiniz için teşekkürler!</p>
                </div>

                <a href={business.slug ? `/book/${business.slug}` : "/"} className="px-6 py-3 font-bold text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
                    Yeni Bir İşlem
                </a>
            </div>
        )
