...
export async function deleteBusinessCompletely(businessId: string) {
    ...
    try {
        ...
        const { data: appointments } = await adminSupabase.from("appointments").select("id").eq("business_id", businessId);
        if (appointments && appointments.length > 0) {
            const apptIds = appointments.map((a: any) => a.id);
            const chunkSize = 200;
            for (let i = 0; i < apptIds.length; i += chunkSize) {
                await adminSupabase.from("appointment_services").delete().in("appointment_id", apptIds.slice(i, i + chunkSize));
            }
        }

        // 2. salon_services
        const { data: salons } = await adminSupabase.from("salons").select("id").eq("business_id", businessId);
        if (salons && salons.length > 0) {
            const salonIds = salons.map((s: any) => s.id);
            await adminSupabase.from("salon_services").delete().in("salon_id", salonIds);
        }

        // 3. Delete dependent records using business_id
        ...
        for (const table of dependentTables) {
            await adminSupabase.from(table).delete().eq("business_id", businessId);
        }

        // 4. Finally delete the business itself
        const { error } = await adminSupabase.from("businesses").delete().eq("id", businessId);

        if (error) {
            return { success: false, error: error.message };
        }

        revalidatePath("/(admin)/superyonetici", "page");
        return { success: true };
    } catch (e: any) {
